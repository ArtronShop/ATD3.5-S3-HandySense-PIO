...
static ArtronShop_SHT45 sht(&Wire);
...
int handySenseSHT45TempRead(float* value) {
    CHECK_SENSOR;

    sht.measure();
    *value = sht.temperature();
        
    return *value < 0 ? -20 : 0;
}

int handySenseSHT45HumiRead(float* value) {
    CHECK_SENSOR;
