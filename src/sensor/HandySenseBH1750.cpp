...
static BH1750 sensor(0x23);

static bool init_sensor = false;
#define CHECK_SENSOR    ({ \
                            if (!init_sensor) {\
                                init_sensor = sensor.begin(BH1750::CONTINUOUS_HIGH_RES_MODE) ? true : false;\
                            }\
                            \
                            if (!init_sensor) {\
                                return -10;\
...
int handySenseBH1750LightRead(float* value) {
    CHECK_SENSOR;

    *value = sensor.readLightLevel() / 1000.0f;
